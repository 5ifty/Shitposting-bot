...
bot = commands.Bot(command_prefix='!!!', description=description)  # you can change the prefix & desc but please link it back to me<3
bot.remove_command('help')   # removes default built in help command, delete this if you want the default one.
...
async def on_ready():
    ...
    print(f'---Logged in---')
    game = "With bad memes| !!!help "  # to avoid syntax errors (Cos im dumb), made it a variable. Change this at your own will, idrc.
    await bot.change_presence(activity=discord.Game(type=3, name=game), status=discord.Status.dnd)  # again with this change at your own will , the status of the bot doesn't matter
...
# please don#t remove these 2 commands , I would love to be credited for making this bot!
@bot.command()
async def source(ctx):
    await ctx.send(f"This is my source code! https://github.com/5ifty/Shitposting-bot\n and to get help join my discord server - https://discord.gg/jAaSeEx")


@bot.command()
async def owner(ctx):
    await ctx.send(f"My owner is **5ifty#0777**, and you can join my home server here - https://discord.gg/jAaSeEx")
...
async def help(ctx):
    ...
    embed.add_field(name="Other", value=f"`about - Shows stats of the bot.\nhelp - Shows this message.\nowner - See the owner of this bot and how to contact him<3\nping - Pong!\nsource - Shows the source code of this bot.`", inline=False)
...
# This not nice part of code handles errors(for owner/Admin purposes only<3)
# Will appear in your CMD.
